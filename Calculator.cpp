...
double calculator(string equation, string prevAnswer) {

	vector<string> outputQueue;
	vector<string> operatorStack;

	//prevAnswer is a pointless variable right now. Will create a better solution sometime later.
	prevAnswer = "0";
	...
	while (outputQueue.at(0).back() == '0') {
		outputQueue.at(0).pop_back();
	}
	if (outputQueue.at(0).back() == '.') {
		outputQueue.at(0).pop_back();
	}

	stringstream number;

	number << outputQueue.at(0);
	return stod(number.str());
